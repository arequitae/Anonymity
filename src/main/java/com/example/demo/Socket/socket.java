...
public class socket {
    ...
    private static socket getEmployee(byte[] bArr) {
        String name = "";
        int id = 0;
        float salary = 0;
        ...
        return new socket(name, id, salary);
     ...
     * 构造并转换
     */
    public socket(String name, int id, float salary) {
        this.name = name;
        this.id = id;
        this.salary = salary;
        ...
        try {
            ...
            while(true){
               ...
               Thread.sleep(2000);
                ...
                socket.getOutputStream().write(new socket("kingfish"+index, 1+index++, 10.99f + index).getbuf());
                socket.getInputStream().read(receive);
                socket ee = getEmployee(receive);
